...
class DictionaryLayer:
    ...
    def lookup(self, candidates: list[str]) -> Optional[GenderResult]:
        ...
        Lookup order (most-specific wins):
          1. All multi-word candidates in full_names  <- compound exceptions first
          2. All single-word candidates normally

        This ensures "naseem akhter" (full_names=F) beats "naseem" (first_names=M).
        """
        # Pass 1: compound (multi-word) candidates against full_names only
        for token in candidates:
            if " " in token and token in self._full:
                entry = self._full[token]
                return self._make_result(entry, token, exact=True)

        # Pass 2: single-word candidates normally (full_names then first_names)
        for token in candidates:
            if " " not in token:
